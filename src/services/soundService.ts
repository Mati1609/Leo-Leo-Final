...
class SoundService {
  ...
  private setupInteractionListeners = () => {
    ...
    const onceHandler = () => {
      // CRÍTICO: crear el AudioContext sincrónicamente dentro del evento de usuario
      // Cualquier await antes de esto rompe el desbloqueo en iOS y Android
      if (!this.audioCtx) {
        const AudioCtx = window.AudioContext || (window as any).webkitAudioContext;
        this.audioCtx = new AudioCtx();
      }
 
      // resume() sin await — el .then() no rompe la cadena de interacción
  ...
  // Sincrónico — solo reanuda si el contexto ya existe
  private initCtx() {
